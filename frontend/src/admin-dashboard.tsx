...
export function AdminDashboard(){
  ...
  return <main>
    ...
    <section className="card history"><h2>Ręczne zwiększenie zapasu</h2><p className="hint">CASP kupuje wybraną pulę od emitenta, a nowe tokeny dzieli 20% hot / 80% cold. Jest to demonstracyjna polityka biznesowa CASP, nie wymóg MiCA. Automatyczne uzupełnianie jest wyłączone.</p><form className="inventory-form" onSubmit={event=>{event.preventDefault();if(Number(inventoryAmount)>0)replenish.mutate()}}><label htmlFor="inventory-amount">Kwota zakupu w USD</label><input id="inventory-amount" value={inventoryAmount} onChange={event=>setInventoryAmount(event.target.value)} inputMode="decimal"/><button className="primary" disabled={replenish.isPending||Number(inventoryAmount)<=0}>{replenish.isPending?"Realizacja…":"Kup pulę od emitenta"}</button></form>{replenish.error&&<p className="error">{replenish.error.message}</p>}<div className="breakdown"><Row label="Docelowy hot wallet" value={formatRaw(plan.data?.targetHotRaw)}/><Row label="Docelowy cold wallet" value={formatRaw(plan.data?.targetColdRaw)}/><Row label="Korekta hot" value={formatDelta(plan.data?.hotDeltaRaw)}/><Row label="Korekta cold" value={formatDelta(plan.data?.coldDeltaRaw)}/></div><p className="policy-note">{plan.data?.policyVersion??"casp-manual-inventory-20-80-v1"} · kalkulator pokazuje także drift wymagający przyszłego transferu między portfelami.</p><div className="table">{replenishments.data?.slice(0,5).map(operation=><article key={operation.operationId}><div><strong>{formatMinor(operation.amountUsdMinor)} USD</strong><small>{operation.operationId}</small></div><span className={`status ${operation.status==="completed"?"completed":""}`}>{operation.status}</span></article>)}</div></section>
